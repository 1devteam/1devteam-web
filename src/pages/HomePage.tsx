        ...
        <div className="container-site py-20 md:py-28 lg:py-32">
          ...
          <div className="mt-9 flex flex-col gap-3 sm:flex-row">
            <Button asChild size="lg">
              <Link to="/work">Inspect the work <ArrowRight className="h-4 w-4" aria-hidden /></Link>
            </Button>
            ...
            {buildAreas.map((area) => (
              <article key={area.title} className="border-t border-[var(--border)] pt-5">
                ...
                <Link to={area.href} className="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-[var(--brand)] hover:underline">
                  {area.cta} <ArrowRight className="h-4 w-4" aria-hidden />
                </Link>
              </article>
            ))}
          </div>
            ...
            {developmentSystems.map((system) => (
              <article key={system.name} className="border-t border-[var(--border)] pt-5">
                ...
                {system.href.startsWith('/#') ? (
                  <a href={system.href} className="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-[var(--brand)] hover:underline">
                    {system.cta} <ArrowRight className="h-4 w-4" aria-hidden />
                  </a>
                ) : (
                  <Link to={system.href} className="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-[var(--brand)] hover:underline">
                    {system.cta} <ArrowRight className="h-4 w-4" aria-hidden />
                  </Link>
                )}
              </article>
          ...
          <div id="architecture-graph" className="scroll-mt-24 mt-12 border-t border-[var(--border)] pt-10">
            <div className="mb-6 max-w-4xl">
              ...
              <p className="mt-3 text-[17px] leading-relaxed text-[var(--text-muted)]">
                Inspect selected real nodes and mapped <code>calls_function</code>, <code>tests_function</code>, and <code>defined_in</code> relationships from Ajenda&apos;s canonical development graph.
              </p>
            </div>
